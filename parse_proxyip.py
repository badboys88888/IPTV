#!/usr/bin/env python3
"""
Cloudflare ProxyIP 筛选 - 完整增强版
- HTTP 连通性（强制 CF-RAY 头）
- 直接 HTTPS 下载测速（淘汰慢节点）
- 自动地理位置 + 输出无测速标签
"""
...
# HTTP 连通性测试（Cloudflare Worker）
TEST_HOST = "cloudflare.snippets1.dpdns.org"
TEST_PATH = "/?ed=2560"

# 测速目标（直接下载，无需 Worker 额外支持）
...
ALLOWED_CODES   = {200, 301, 302, 403}   # 白名单状态码
...
def has_cf_headers(response_bytes):
    """检查是否包含 Cloudflare 典型头部（必须存在 cf-ray）"""
    try:
        headers = response_bytes.split(b"\r\n\r\n")[0].lower()
    except:
        return False
    # 必须包含 cf-ray，这是 Cloudflare 的核心标识
    return b"cf-ray" in headers

def http_connectivity_measure(ip, port):
    """通过 ProxyIP 访问 Worker，验证 CF-RAY 头"""
    ...
    def _try(use_tls):
        ...
        try:
            ...
            # 严格检查：所有状态码（包括200/403）都必须有 cf-ray
            if not has_cf_headers(resp):
...
def main():
    print(f"🚀 增强筛选（强制 CF-RAY 头 + 速度测试 {MIN_SPEED_KBPS} KB/s）", flush=True)
    proxies = read_csv()
    if not proxies:
        return
    passed = []
